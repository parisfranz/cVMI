...
#mdatname='/cds/home/p/paris/reconVMI_clean/recon X510 Dec/Ms_Aug10_112.npy'
#specdatname ='/cds/home/p/paris/reconVMI_clean/recon X510 Dec/specs_Aug10_112.npy'
...
idlist = [14,22,30,33,42,47,53,55,56,57,64,70,72,82,85,86,90,93,102,103,104,155]
if order == -1: idlist = np.flip(idlist)
for k in idlist:
    for t in range(5):
        tt = np.copy(t)
        savefilename = '/reg/d/psdm/tmo/tmox51020/results/paris/recon/torch_Aug11_112_half/pytorchrecon_'+str(k)+'_Up'+str(Ups[0])+'_seed'+str(tt)+'.npy'
...
#             spec_in = f(xs)
         ...
         #####
            ...
            eps = []
            M_wght = 1
            Spec_wght = 15 #5 #15 #12 #1 #0.75 #0.8
            Smooth_wght = 0.00001
            for epoch in range(int(n_epochs)):
                #M,Spec, Smooth
                
                
                ll=loss_tot_half(M,Q,Nb,Bpr,Bpi,spec_in, alphaw_r, alphaw_i,M_wght,Spec_wght,Smooth_wght,thresh)
                loss_tot_half(M,Q,Nb,Bpr,Bpi,spec_in, alphaw_r, alphaw_i,M_wght,Spec_wght,Smooth_wght,thresh).backward()
                ...
                if epoch%100==0:
                    ...
                    costM.append(loss_M_half(M,Q,Nb,Bpr,Bpi,thresh).detach().numpy()[()]*M_wght)
